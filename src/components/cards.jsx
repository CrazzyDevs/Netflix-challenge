        ...
        <div className="grid lg:grid-cols-3 md:grid-cols-3 gap-10 place-items-center justify-center">
          {data.map(({ id, name, borderColor }) => (
            <Link
              to={`/${name}`}
              key={id}
              className={`lg:w-[300px] md:w-[200px] w-[250px] h-[250px] mx-auto border rounded-[30px] hover:shadow-xl hover:border-2 cursor-pointer transition-all duration-300 shadow-lg items-center justify-center`}
              style={{ borderColor }}
            >
